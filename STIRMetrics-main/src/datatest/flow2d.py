...
#import STIRLoader.STIRLoader as STIRLoader
from STIRLoader import getclips
import random
import torch
import argparse
...
modeldict = {
    ...
    "MFTWAFT" : mftwaft.MFTWAFTTracker,
}
...
def trackanddisplay(
    ...
    for data in tqdm(dataloaderiter):
        ...
        with LogLatency(name=sequence_name):
            coords, _ = model.trackpoints2D(pointlist, ims_ori_pair)
            pointlist = coords
...
if __name__ == "__main__":
    ...
    datasets = getclips(datadir=args.datadir)
    random.seed(1249)
    ...
    for ind, dataset in enumerate(datasets[:num_data]):
        try:
            ...
            if pointlist_start.shape[0] < 1:
                print(f"Skipping dataset {dataset.dataset.basename}, start points empty")
                continue
            if not args.ontestingset:
                print(f"Skipping dataset {dataset.dataset.basename}, ontestingset false")
            ...
            if args.showvis:
                print(f"Skipping dataset {dataset.dataset.basename}, showvis true")
            ...
            if args.showvis:
                ...
                for pt, displacement in zip(end_estimates, displacements):
                    ...
                    pt = np.array(pt).astype(int)
                    #pt = pt.astype(int)
                    #pt = pt.detach().astype(int)
                    #displacement = displacement.detach().cpu().numpy().astype(int)
            ...
            print(f"error on dataset load: {e}, continuing")
    ...
    with open(
        ...
        def tensor_to_list(obj):
            ...
            if isinstance(obj, list):
                return [tensor_to_list(o) for o in obj]
            if isinstance(obj, dict):
                return {k: tensor_to_list(v) for k, v in obj.items()}
            return obj

        json.dump(tensor_to_list(positionlists), fp)
