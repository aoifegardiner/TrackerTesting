...
class MFTTracker:
    ...
    def trackpoints2D(self, pointlist, impair):
        ...
        def convert_to_np_bgr(im):
            ...
            im = cv2.resize(im, self.internalimsize)  
            return im

        if not self.initialized:
            device = torch.device("cpu")
            self.queries = torch.from_numpy(pointlist).float().to(device) ## check format
            self.queries = self.queries / self.scale
            meta = self.tracker.init(convert_to_np_bgr(impair[0]))
            self.initialized = True
        meta = self.tracker.track(convert_to_np_bgr(impair[1])) 
