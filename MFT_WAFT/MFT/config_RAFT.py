...
def load_config(path):
    """Safely load a config module, compatible with Python 3.8+."""
    ...
    # --- Compatibility block for importlib.resources.files() ---
    try:
        # Python 3.9+
        package_path = importlib.resources.files(__package__)
    except AttributeError:
        # Python 3.8 fallback: use context manager
        import importlib.resources as pkg_resources
        with pkg_resources.path(__package__, "") as p:
            package_path = Path(p)

    # --- Build absolute config path ---
    path = Path(package_path, path)
    assert path.exists(), f"Config file does not exist: {path}"

    # --- Load the config as a Python module dynamically ---
    spec = importlib.util.spec_from_file_location("tracker_config", path)
    foo = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(foo)

    # --- Return the tracker configuration object ---
