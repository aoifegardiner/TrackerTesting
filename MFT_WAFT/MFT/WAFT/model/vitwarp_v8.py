...
from MFT_WAFT.MFT.WAFT.model.backbone.depthanythingv2 import DepthAnythingFeature
from MFT_WAFT.MFT.WAFT.model.backbone.vit import VisionTransformer, MODEL_CONFIGS

from MFT_WAFT.MFT.WAFT.utils.utils import coords_grid, Padder, bilinear_sampler
...
class ViTWarpV8(nn.Module):
    ...
    def normalize_image(self, img):
        ...
        '''
        tf = torchvision.transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                              std=[0.229, 0.224, 0.225],
                                              inplace=False)
    
        if isinstance(img, np.ndarray):
            if img.ndim == 3:
                # single image HWC → CHW
                img = torch.from_numpy(img).permute(2, 0, 1).float().unsqueeze(0)
            elif img.ndim == 4:
                # batch of images BHWC → BCHW
                img = torch.from_numpy(img).permute(0, 3, 1, 2).float()
        elif isinstance(img, torch.Tensor):
            # if input is already BCHW, do nothing
            if img.ndim == 3:
    ...
    def forward(self, image1, image2, iters=None, flow_gt=None):
        """Estimate optical flow between a pair of frames at training resolution."""
        ...
        # Initialize flow at half resolution
        ...
        # Compute NF loss if ground truth is provided
...
#    def forward(self, image1, image2, iters=None, flow_gt=None):
#        """Estimate optical flow between pair of frames at training resolution."""
#        H_orig, W_orig = image1.shape[:2]
#
#        if iters is None:
#            iters = self.args.iters
...
#        image2 = self.normalize_image(image2)
#
#        # Padding (factor=112 for model compatibility)
#        padder = Padder(image1.shape, factor=112)
#        image1 = padder.pad(image1)
#        image2 = padder.pad(image2)
#
#        #print('inter image1: ', image1.min(), image1.max(), image1.mean(), image1.std())
#        #print('inter image2: ', image2.min(), image2.max(), image2.mean(), image2.std())
#
#        flow_predictions = []
#        info_predictions = [] 
#
...
#            #coords_final = flow_predictions[-1]  
#            #coords_final[:, 0, :, :] *= W_orig / coords_final.shape[3] 
#            #coords_final[:, 1, :, :] *= H_orig / coords_final.shape[2]
#
#        if flow_gt is not None:
#            nf_predictions = []
#            for i in range(len(info_predictions)):                 
