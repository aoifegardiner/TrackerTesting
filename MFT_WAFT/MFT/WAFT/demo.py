...
@torch.no_grad()
def demo_stir(model, args, device=torch.device('cuda')):
    dataset = STIR()
    image1, image2, flow_gt, valid = dataset[289992]
    image1 = image1[None].to(device)
    image2 = image2[None].to(device)
    flow_gt = flow_gt[None].to(device)
    demo_data('stir', args, model, image1, image2, flow_gt, valid=valid)
...
def main():
    ...
    parser.add_argument('--dataset', help='dataset to evaluate on', choices=['chairs', 'sintel', 'spring', 'tartanair', 'kitti', 'stir'], required=True, type=str)
