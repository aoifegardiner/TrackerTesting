...
def load_ckpt(model, path):
    ...
    #model = torch.jit.load("ckpts/raft_pointtrackSTIR.pt")
    #model.eval()

    # Then run forward passes directly:
    #output = model(input_tensor)

#def load_ckpt(model, path):
#    """Load RAFT checkpoint into WAFT model"""
#    # Try normal PyTorch checkpoint load first
#    try:
#        state_dict = torch.load(path, map_location=torch.device('cpu'))
#        if not isinstance(state_dict, dict):
#            # TorchScript model, get its state dict
#            state_dict = torch.jit.load(path, map_location='cpu').state_dict()
#    except RuntimeError:
#        # Fallback if TorchScript-only checkpoint
#        state_dict = torch.jit.load(path, map_location='cpu').state_dict()
#
#    # Filter keys to match WAFT architecture
#    waft_state_dict = model.state_dict()
#    compatible_state_dict = {k: v for k, v in state_dict.items() if k in waft_state_dict}
#
#    # Load compatible weights
#    model.load_state_dict(compatible_state_dict, strict=False)
#
#    print(f"Loaded {len(compatible_state_dict)} / {len(waft_state_dict)} layers from RAFT into WAFT")
#    return model
