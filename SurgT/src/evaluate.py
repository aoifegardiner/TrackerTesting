...
class AnchorResults:
    ...
    def calculate_bbox_metrics(self, bbox1_gt, bbox1_p, bbox2_gt, bbox2_p, is_track_fail_2d, is_track_fail_3d):
        ...
        if not is_track_fail_2d:
            ...
            if self.n_misses_successive_2d > self.n_misses_allowed:
                # Instead of crashing, mark a failure and reset
                self.n_misses_successive_2d = 0
                flag_track_fail_2d = True
...
def draw_bb_in_frame(im1, im2, bbox1_gt, bbox1_p, bbox2_gt, bbox2_p, is_difficult, thck):
    ...
    if im2 is None:
        return im1  # <--- ✅ just return left image if no right stream
...
def assess_anchor(time, v, anch, ar, kss, is_visualization_off, config_results):
    """
    Run MFT tracking (left camera only) from the FIRST anchor until the end of the video.
    Saves all predicted bounding boxes to one file and an MP4 video with overlays.
    """
    import os
    import pickle
    import cv2 as cv
...
#    writer = None  # we'll initialize once we know the frame size
    ...
    print("\n=== Running MFT tracker on Case 1, Video 1 (LEFT only) ===")
    ...
    while v.cap.isOpened():
        ...
        # Split stereo frame but ignore right stream
...
#            break
        ...
        if t is None:
            ...
            if bbox1_gt is not None and not is_difficult:
                if v.is_bbox_inside_image(bbox1_gt, bbox1_gt):
                    ...
                    print(f"[INIT] Tracker initialized at frame {v.frame_counter} with bbox {bbox1_gt}")
                    kss.add_ss()
            continue
        ...
        if bbox1_p is None:
            print(f"[WARN] No prediction at frame {v.frame_counter}")
            continue
...
def print_results(summary_title, stats):
    """
    Concise summary of evaluation metrics.
    """
    lines = [f"{summary_title}"]
    
    # 2D metrics
    if stats.n_f_2d != 0:
        lines.append(f"  2D -> Acc: {stats.acc:.3f}, Err: {stats.err_2d:.1f} ± {stats.err_2d_std:.1f} px, Rob: {stats.rob_2d:.3f}")
    else:
        lines.append("  2D -> No valid frames to evaluate.")
    
    # 3D metrics
    if stats.n_f_3d != 0:
        lines.append(f"  3D -> Err: {stats.err_3d:.2f} ± {stats.err_3d_std:.1f} mm, Rob: {stats.rob_3d:.3f}")
    else:
        lines.append("  3D -> No valid frames to evaluate.")
...
""" def calculate_results(config, valid_or_test, is_visualization_off):
    ...
    if config_data["is_to_evaluate"]:
        ...
        if cases:  # make sure list is not empty
            case = cases[0]   # only the first case
            stats_case = Statistics()
            print("\n\t{}".format(case.case_id))

            if case.case_samples:
                cs = case.case_samples[0]   # only the first case sample (video)
                stats_video = Statistics()

                print(f"Video path: {cs.case_sample_path}")
                print(f"Anchors: {cs.anchors}")
                print(f"GT file: {cs.case_sample_path}/gt_rectified_0.yaml")
                gt_file = os.path.join(cs.case_sample_path, "gt_rectified_0.yaml")
                # Load GT manually
                import yaml
                with open(gt_file, "r") as f:
                    gt_data = yaml.load(f, Loader=yaml.FullLoader)  # instead of safe_load

                print(f"GT frame range: {min(gt_data.keys())} → {max(gt_data.keys())}")

                calculate_results_for_video(time, rank, stats_video,
                                            cs.anchors, cs.case_sample_path,
                                            is_to_rectify, config_results, is_visualization_off)
                print_results("\t\t{}".format(cs.case_sample_path), stats_video)
                stats_case.append_stats(stats_video)

            # Get results just for this case
            stats_case.merge_stats()
            print_results("\t\tWeighted average, ", stats_case)
            stats_case_all.append_stats(stats_case)
...
def calculate_results(config, valid_or_test, is_visualization_off):
    ...
    if config_data["is_to_evaluate"]:
        ...
        for case in cases:
            ...
            # Go through case samples (videos)
            for cs in case.case_samples:
                stats_video = Statistics()  # Statistics for a video

                # Debug info
                print(f"Video path: {cs.case_sample_path}")
                print(f"Anchors: {cs.anchors}")

                # Optional: load GT to check frame range
                import os, yaml
                gt_file = os.path.join(cs.case_sample_path, "gt_rectified_0.yaml")
                ...
                calculate_results_for_video(
                    time, rank, stats_video, cs.anchors,
                    cs.case_sample_path, is_to_rectify,
                    config_results, is_visualization_off
                )
                print_results("\t\t{}".format(cs.case_sample_path), stats_video)
                stats_case.append_stats(stats_video)

            # Get results for all videos in the case
            stats_case.merge_stats()
            print_results("\t\tWeighted average, ", stats_case)
            stats_case_all.append_stats(stats_case)

        # Final stats across all cases
        stats_case_all.merge_stats()
        print('{} final score:'.format(valid_or_test).upper())
        eao = rank.calculate_eao_score()
        print_results("\tEAO:{:.3f}".format(eao), stats_case_all)
